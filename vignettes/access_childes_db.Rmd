...
**Technical note 1**: As of childesr 0.3, data are retrieved from the versioned [childes-db dataset on Redivis](https://redivis.com/datapages/datasets/childes_db) (using the [`redivis`](https://apidocs.redivis.com/client-libraries/redivis-r) R package) rather than from a MySQL server, so no database connection is needed. The dataset is public, so no Redivis account is required; if you have a Redivis API token, you can set it in the `REDIVIS_API_TOKEN` environment variable. The `connection` argument of the `get_` functions and the `connect_to_childes()` / `clear_connections()` functions are deprecated and retained only for backwards compatibility.
