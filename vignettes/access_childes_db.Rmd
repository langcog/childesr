...
For example, if we wanted to get the summary statistics for Adam's production data, you could run the following call. 
...
The `get_contexts()` function returns a table with a row for each utterance that is within a given window of the utterances that contain the given token(s).

For example, you could run the following call to get all utterances containg "dog" for Adam in the Brown corpus, along with the preceeding utterance and succeeding utterance for each one.

```{r}
d_adam_context <- get_contexts(corpus = "Brown",
                               role = "target_child",
                               target_child = "Adam",
                               token = "dog",
                               window = c(1, 1))
