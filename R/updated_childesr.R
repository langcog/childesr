...
#' }
get_contexts_u <- function(collection = NULL, language = NULL, corpus = NULL,
                        role = NULL, role_exclude = NULL, age = NULL,
  ...
  utterance_orders <-
    tibble(
      ...
      utterance_order = utterance_orders[rep(1:nrow(utterance_orders), each=sum(window) + 1),]$utterance_order-(-window[1]:window[2])
    )
